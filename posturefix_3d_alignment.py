"""PostureFix Minimal - Single-file posture analysis.

Features: MediaPipe pose detection, view-aware feature-based posture scoring,
Kalman tracking, optional 3D rigid alignment, real-time overlay, and text session reports.

Usage: python posturefix_features.py
"""
...
class PoseData:
    ...
    world_keypoints: Optional[np.ndarray] = None  # (33, 4) - x, y, z, visibility
...
class PostureAnalyzer:
    ...
    def _soft_score(deviation: float, tolerance: float) -> float:
        # Smooth, nonlinear falloff: near the target, errors are forgiven; large errors drop faster.
...
class PostureFixApp:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("PostureFix Feature Scoring")
        self.root.geometry("960x620")

        self.estimator = PoseEstimator(320, 240)
        self.analyzer = PostureAnalyzer()
        self.tracker = OpticalFlowTracker()
        self.kalman = KalmanKeypointTracker()
